...
(function ($) {
  ...
  var defaultOptions = {
    ...
    itemText: function(item) {
        if (/^[a-z0-9._-]+@[a-z0-9._-]+\.[a-z]{2,6}$/.test(item)){
  ...
  function TagsInput(element, options) {
    ...
    this.type = element.hasAttribute('type') ? this.$element.attr('type') : 'email';
    this.inputSize = Math.max(1, this.placeholderText.length);

    this.$container = $('<div class="bootstrap-tagsinput"></div>');
    this.$input = $('<input type="' + this.type + '" name="' + this.name + '" id="' + this.id + '"placeholder="' + this.placeholderText + '"/>').appendTo(this.$container);
  ...
  TagsInput.prototype = {
    ...
    add: function(item, dontPushVal, options) {
      ...
      if (typeof item === "object" && !self.objectItems || self.options.itemText(item) == null)
        return alert('Allow is email only !');
